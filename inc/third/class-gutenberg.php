...
// Start and run class.
if ( ! class_exists( 'subetuwebWP_Gutenberg_Editor' ) ) {

	/**
	 * subetuwebWP Gutenberg Class
	 */
	class subetuwebWP_Gutenberg_Editor {
		...
		public function elements() {
			...
			return apply_filters(
				'subetuweb_gutenberg_typography_tags',
				array(
					'body'            => array(
		...
		public function loop( $return = 'css' ) {
			...
			if ( ! empty( $elements ) ) {
				...
				if ( 'css' === $return && ! empty( $css ) ) {
					$css = '/* subetuwebWP Gutenberg Style */' . $css;
					return $css;
				}
		...
		public function gutenberg_editor_style( $output ) {
			...
			if ( ! is_null( get_current_screen() ) && get_current_screen()->is_block_editor() ) {
				...
				$heading_tag = get_theme_mod( 'subetuweb_single_post_heading_tag', 'h2' );
				...
				$background_layout           = get_theme_mod( 'subetuweb_main_layout_style', 'wide' );
				$background_color            = get_theme_mod( 'subetuweb_background_color', '#ffffff' );
				$boxed_separate_bg_color     = get_theme_mod( 'subetuweb_boxed_inner_bg', '#ffffff' );
				...
				if ( ! empty( $gutenberg_css ) ) {
					echo '<style id="subetuweb-gutenberg-style"> ' . esc_attr( $gutenberg_css ) . '</style>';
				}
			}
...
return new subetuwebWP_Gutenberg_Editor();

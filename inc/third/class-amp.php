...
// Start and run class.
if ( ! class_exists( 'subetuwebWP_AMP' ) ) {

	/**
	 * subetuwebWP AMP Setup
	 */
	class subetuwebWP_AMP {
		...
		public function __construct() {
			...
			add_action( 'wp', array( $this, 'subetuwebwp_amp_config' ) );
		 ...
		 * @return void
		 */
		public function subetuwebwp_amp_config() {

			if ( ! subetuwebWP_Theme_Class::subetuwebwp_is_amp() ) {
				return;
			}

			add_filter( 'subetuwebwp_schema_location_html', array( $this, 'subetuwebwp_body_id' ) );
			add_filter( 'subetuwebwp_menu_toggle_data_attrs', array( $this, 'subetuwebwp_menu_taggle_attrs' ) );
			add_action( 'wp_footer', array( $this, 'render_amp_states' ) );
			add_filter( 'subetuwebwp_attrs_mobile_dropdown', array( $this, 'subetuwebwp_dropdown_menu_wrapper' ) );
			add_filter( 'subetuwebwp_attrs_mobile_fullscreen', array( $this, 'subetuwebwp_fullscreen_menu_wrapper' ) );
			add_filter( 'subetuwebwp_attrs_mobile_fullscreen_close', array( $this, 'subetuwebwp_fullscreen_menu_close' ) );
			add_filter( 'walker_nav_menu_start_el', array( $this, 'mobile_submenu_icon' ), 20, 4 );
			add_filter( 'subetuwebwp_toggle_submenu_icon', array( $this, 'toggle_submenu_icon' ), 20, 2 );
			add_filter( 'owp_attr_submenu-toggle', array( $this, 'menu_toggle_button' ), 20, 3 );
			add_filter( 'subetuwebwp_attrs_search_bar', array( $this, 'subetuwebwp_menu_search_bar' ) );
			add_filter( 'subetuwebwp_attrs_overlay_search_bar', array( $this, 'subetuwebwp_menu_overlay_search_bar' ) );
			add_filter( 'subetuwebwp_attrs_nav_search_bar', array( $this, 'subetuwebwp_nav_item_search_bar' ) );
			add_filter( 'subetuwebwp_attrs_main_nav', array( $this, 'subetuwebwp_attrs_nav_wrapper' ) );
			add_filter( 'subetuwebwp_attrs_vertical_header_style', array( $this, 'subetuwebwp_attrs_vertical_header_toggle' ) );
			add_filter( 'subetuweb_head_css', array( $this, 'subetuwebwp_amp_css' ) );

			// Woocommerce AMP.
			if ( subetuwebWP_WOOCOMMERCE_ACTIVE ) {

				add_filter( 'subetuweb_menu_cart_style', array( $this, 'subetuwebwp_amp_mini_cart_style' ) );
		 ...
		 * @return String
		 */
		public function subetuwebwp_body_id( $schema ) {
			return $schema . 'id="subetuwebwp-body"';
		}
		 ...
		 */
		public function subetuwebwp_amp_css( $output ) {
			...
			$amp_menu_breakpoint        = get_theme_mod( 'subetuweb_mobile_menu_breakpoints', '959' );
			$amp_menu_custom_breakpoint = get_theme_mod( 'subetuweb_mobile_menu_custom_breakpoint' );
			$amp_vheader_style_width    = get_theme_mod( 'subetuweb_vertical_header_width', '300' );
			...
			.fullscreen-mobile .subetuwebwp-mobile-menu-icon a.mobile-menu.opened .subetuwebwp-text {
				display: inherit;
			}';
			...
			if ( subetuwebWP_WOOCOMMERCE_ACTIVE ) {
				...
				.woocommerce .subetuwebwp-grid-list {
					display: none;
				}
			...
			// Return CSS.
			if ( ! empty( $css ) ) {
				$output .= '/* subetuwebWP AMP CSS */' . $css;
			}
		 ...
		 * @return string
		 */
		public function subetuwebwp_menu_taggle_attrs( $input ) {
			$input .= ' on="tap:AMP.setState( { owpAmpMenuExpanded: ! owpAmpMenuExpanded } ),subetuwebwp-body.toggleClass(class=mobile-dropdown)" ';
		 ...
		 * @return string
		 */
		public function subetuwebwp_dropdown_menu_wrapper( $input ) {
		 ...
		 * @return string
		 */
		public function subetuwebwp_fullscreen_menu_wrapper( $input ) {
			$input .= ' [class]="\'clr\' + ( owpAmpMenuExpanded ? \' opened\' : \'\' )" ';
			$input .= ' aria-expanded="false" ';
			$input .= ' [aria-expanded]="owpAmpMenuExpanded ? \'true\' : \'false\'" ';
		 ...
		 * @return string
		 */
		public function subetuwebwp_fullscreen_menu_close( $input ) {
			$input .= ' on="tap:AMP.setState( { owpAmpMenuExpanded: ! owpAmpMenuExpanded } ),subetuwebwp-body.toggleClass(class=clr)" ';
		 ...
		 * @return string
		 */
		public function subetuwebwp_attrs_vertical_header_toggle( $input ) {
			$input .= ' on="tap:AMP.setState( { owpAmpVhMenuExpanded: ! owpAmpVhMenuExpanded } ),subetuwebwp-body.toggleClass(class=amp-vertical-header)" ';
		...
		public function menu_arrow_button_markup( $item_output, $item ) {
			$item_output  = apply_filters( 'subetuwebwp_toggle_submenu_icon', $item_output, $item );
			$item_output .= '<button ' . owp_attr(
				...
				$item
			) . '><span class="screen-reader-text">' . __( 'Menu Toggle', 'subetuwebwp' ) . '</span></button>';
		 ...
		 * @return string
		 */
		public function subetuwebwp_nav_item_search_bar( $input ) {
		 ...
		 * @return string
		 */
		public function subetuwebwp_menu_search_bar( $input ) {
		 ...
		 * @return string
		 */
		public function subetuwebwp_menu_overlay_search_bar( $input ) {
		...
		/**
		 * Add AMP attributes to the subetuwebwp nav wrapper.
		 *
		 * @param string $input the data attrs already existing in search toggle.
		 * @return string
		 */
		public function subetuwebwp_attrs_nav_wrapper( $input ) {

			if ( 'header_replace' !== get_theme_mod( 'subetuweb_menu_search_style', 'drop_down' ) ) {
				return;
			}
		 ...
		 */
		public function subetuwebwp_amp_mini_cart_style( $style ) {
			$style = 'custom_link';
			return $style;
		}

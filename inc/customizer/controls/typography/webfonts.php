 ...
 * @since 1.0.0
 */
function subetuwebwp_standard_fonts() {
	return apply_filters(
		'subetuweb_standard_fonts_array',
		array(
 ...
 * @since 1.0.0
 */
function subetuwebwp_enqueue_google_font( $font ) {

	// Return if disabled
	if ( true == get_theme_mod( 'subetuweb_disable_google_font', false ) ) {
		return;
	}

	// Get list of all Google Fonts
	$google_fonts = subetuwebwp_google_fonts_array();
	...
	// Subset
	$get_subsets = get_theme_mod( 'subetuweb_google_font_subsets', array( 'latin' ) );
	$subsets     = '';
	...
	$weights = apply_filters( 'subetuweb_google_font_enqueue_weights', $weights, $font );
	$italics = apply_filters( 'subetuweb_google_font_enqueue_italics', true );
	...
	$url = apply_filters( 'subetuwebwp_enqueue_google_font_url', $url, $handle );

	// Enqueue style
	wp_enqueue_style( 'subetuwebwp-google-font-' . $handle, $url, false, false, 'all' );
 ...
 *
 */
add_filter( 'subetuwebwp_enqueue_google_font_url', 'subetuwebwp_webfonts_enqueue', 20, 2 );
function subetuwebwp_webfonts_enqueue( $src, $font_name ) {
	if ( true != get_theme_mod( 'subetuweb_local_google_font', false ) ) {
		return $src;
	}

	$font_style = subetuweb_get_google_font_css( $src );
	$font_style = explode( "\n", $font_style );

	if ( is_array( $font_style ) && ! empty( $font_style ) ) {

		$webfonts_dir_path     = subetuwebwp_get_local_webfonts_data_dir( $font_name );
		$webfonts_css_dir_path = subetuwebwp_get_local_webfonts_css_data_dir( $font_name );
 ...
 *
 */
function subetuweb_get_google_font_css( $url ) {
	$url = 'https:' . $url;
	$ch  = curl_init();
 ...
 *
 */
function subetuwebwp_get_local_webfonts_data_dir( $file_name ) {
	$upload      = wp_upload_dir();
	$uploads_dir = 'subetuwebwp-webfonts';
 ...
 *
 */
function subetuwebwp_get_local_webfonts_css_data_dir( $file_name ) {
	$upload      = wp_upload_dir();
	$uploads_dir = 'subetuwebwp-webfonts-css';

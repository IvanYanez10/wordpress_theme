 ...
 * @return void
 */
function subetuwebwp_breadcrumb_trail( $args = array() ) {
	...
	if ( function_exists( 'yoast_breadcrumb' )
		...
		if ( $breadcrumbs_position = get_theme_mod( 'subetuweb_breadcrumbs_position' ) ) {
	...
	if ( ! is_object( $breadcrumb ) )
		$breadcrumb = new subetuwebWP_Breadcrumb_Trail( $args );
 ...
 * @access public
 */
class subetuwebWP_Breadcrumb_Trail {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `subetuwebWP_Breadcrumb_Trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see subetuwebWP_Breadcrumb_Trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see subetuwebWP_Breadcrumb_Trail::set_post_taxonomy()
	...
	public function get_trail() {
		...
		$breadcrumb    = '';
		$separator     = apply_filters( 'subetuwebwp_breadcrumb_separator', get_theme_mod( 'subetuweb_breadcrumb_separator', '>' ) );
		...
		if ( 0 < $item_count ) {
			...
			// Postion class.
			$p_class = '';
			if ( '' !== get_theme_mod( 'subetuweb_breadcrumbs_position' ) ) {
				$p_class = ' position-' . get_theme_mod( 'subetuweb_breadcrumbs_position' );
			}
	...
	protected function set_labels() {
		...
		$defaults = array(
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'subetuwebwp' ),
			'home'                => get_theme_mod( 'subetuweb_breadcrumb_translation_home', esc_html__( 'Home', 'subetuwebwp' ) ),
			'error_404'           => get_theme_mod( 'subetuweb_breadcrumb_translation_error', esc_html__( '404 Not Found', 'subetuwebwp' ) ),
			'archives'            => esc_html__( 'Archives', 'subetuwebwp' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => get_theme_mod( 'subetuweb_breadcrumb_translation_search', esc_html__( 'Search results for', 'subetuwebwp' ) ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'subetuwebwp' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'subetuwebwp' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'subetuwebwp' ),
	...
	protected function add_network_home_link() {
		...
		// Home item
		$setting = get_theme_mod( 'subetuweb_breadcrumb_home_item', 'icon' );
		...
		if ( is_customize_preview()
			|| 'icon' == $setting ) {
			$icon = subetuwebwp_icon( 'home', false );
		}
	...
	protected function add_singular_items() {
		...
		// Post type taxonomy
		$posts_tax 		= get_theme_mod( 'subetuweb_breadcrumb_posts_taxonomy', 'category' );
		$products_tax 	= get_theme_mod( 'subetuweb_breadcrumb_products_taxonomy', 'shop' );
		$portfolio_tax 	= get_theme_mod( 'subetuweb_breadcrumb_portfolio_taxonomy', 'subetuweb_portfolio_category' );
		...
		// If the post type is 'subetuweb_portfolio'.
		else if ( class_exists( 'subetuweb_Portfolio' )
			&& 'subetuweb_portfolio' === $post->post_type
			&& 'none' != $portfolio_tax ) {
		...
		if ( true == get_theme_mod( 'subetuweb_breadcrumb_show_title', true )
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'subetuwebwp' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'subetuwebwp' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'subetuwebwp' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'subetuwebwp' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'subetuwebwp' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'subetuwebwp' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'subetuwebwp' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'subetuwebwp' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'subetuwebwp' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'subetuwebwp' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'subetuwebwp' ) ) ) );

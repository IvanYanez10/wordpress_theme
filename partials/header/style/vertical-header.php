...
// Get template.
$template = get_theme_mod( 'subetuweb_vertical_header_template' );
...
// Get bottom template.
$bottom_template = get_theme_mod( 'subetuweb_vertical_header_bottom_template' );
...
<div id="site-header-inner" class="<?php echo esc_attr( $classes ); ?>">
	...
	if ( ! empty( $template ) ) {
		...
		if ( subetuwebWP_ELEMENTOR_ACTIVE && $elementor ) {
			...
			// If Elementor.
			subetuwebWP_Elementor::get_vertical_header_content();
		...
		<?php
		// Search form.
		if ( true === get_theme_mod( 'subetuweb_vertical_header_search_form', true ) ) {
		...
		if ( ! empty( $bottom_template ) ) {
			...
			if ( subetuwebWP_ELEMENTOR_ACTIVE && $bottom_elementor ) {

				// If Elementor.
				subetuwebWP_Elementor::get_vertical_header_bottom_content();

			} elseif ( subetuwebWP_BEAVER_BUILDER_ACTIVE && ! empty( $bottom_template ) ) {

...
if ( subetuwebWP_LIFTERLMS_ACTIVE ) {
	...
	if ( is_membership() && ! ( get_theme_mod( 'subetuweb_llms_membership_image', false ) ) ) {
		return;
	}
}

// Define image alt text usage status.
$sp_seo_set = get_theme_mod( 'subetuweb_enable_sp_fimage_alt', false );
...
$sp_fimage_alt = ( false === $sp_seo_set || ( true === $sp_seo_set && ! $spfe_img_alt ) ) ? subetuwebwp_theme_strings( 'owp-string-current-read', false ) . ' ' . get_the_title() : $spfe_img_alt;
